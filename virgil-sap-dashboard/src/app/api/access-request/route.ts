...
export async function POST() {
  try {
    ...
    const me = await prisma.users.findUnique({ where: { email: userEmail } });
    if (!me)
    ...
    if (!me.organizationId) {
      ...
      return NextResponse.json(
        {
          error: "No organization found for your account. Contact your admin.",
        },
        { status: 400 }
      );
    }
    ...
    const invite = await prisma.org_invites.upsert({
      where: {
        organizationId_email: { organizationId: org.id, email: me.email },
      },
